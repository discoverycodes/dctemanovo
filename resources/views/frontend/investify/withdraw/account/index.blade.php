                                ...
                                <div class="site-table-list">
                                    <div class="site-table-col">
                                        <div class="account-description">
                                            ...
                                            <div class="content" style="display: flex;flex-direction: column;flex-wrap: wrap;align-items: center;">
                                            @php
                                                $credentials = json_decode($account->credentials, true);
                                                $walletName = $credentials['Apelido da Carteira']['value'] ?? 'Conta sem nome';

                                                $busca_logo = \App\Models\WithdrawMethod::where('tnx', $account->withdraw_method_tnx)->first();
                                            @endphp
                                                <img src="{{asset('assets/'.$busca_logo->icon)}}" width="50px" alt="{{ $account->method_name }}" style="max-width: 40px !important;"><h4 class="title sidecar-text">{{ $account->method_name }}</h4>
                                                <h5 class="title sidecar-text">{{ $walletName }}</h5>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="site-table-col">
                                        <div class="action-btn-wrap">
                                            <a class="action-btn primary-btn"
                                                href="{{ route('user.withdraw.account.edit',$account->tnx) }}">
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
